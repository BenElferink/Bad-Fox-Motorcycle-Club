      ...
      case 'POST': {
        ...
        body.payload.forEach(async (payloadItem, idx) => {
          ...
          payloadItem.tx.output_amount.forEach((outputAmountItem) => {
            if (outputAmountItem.unit.indexOf(BAD_MOTORCYCLE_POLICY_ID) === 0) {
          ...
          for (let i = 0; i < assetTxMatrix.length; i++) {
            ...
            if (EXCLUDE_ADDRESSES.includes(from)) {
              ...
              if (!fromWallet) {
                ...
                if (!fromWallet) {
                  const assets = await blockfrost.getAssetIdsWithStakeKey(sKey, BAD_MOTORCYCLE_POLICY_ID)
                  const newWallet = new Wallet({
                    stakeKey: sKey,
                    addresses: [from],
                    assets: {
                      [BAD_MOTORCYCLE_POLICY_ID]: assets.filter((str) => str !== assetId),
                    },
                  })
                  ...
                  fromWallet.assets[BAD_MOTORCYCLE_POLICY_ID] = fromWallet.assets[BAD_MOTORCYCLE_POLICY_ID].filter(
                    (str) => str !== assetId
                  )
                }
              ...
              if (!toWallet) {
                ...
                if (!toWallet) {
                  ...
                  const newWallet = new Wallet({
                    ...
                    addresses: [to],
                    assets: {
                      [BAD_MOTORCYCLE_POLICY_ID]: assets,
                    },
                  })
                  ...
                  toWallet.assets[BAD_MOTORCYCLE_POLICY_ID] = toWallet.assets[BAD_MOTORCYCLE_POLICY_ID].includes(
                    assetId
                  )
                    ? toWallet.assets[BAD_MOTORCYCLE_POLICY_ID]
                    : [...toWallet.assets[BAD_MOTORCYCLE_POLICY_ID], assetId]
